...
Copy this entire file to use in another project.

---

## File 1: `client/src/contexts/ThemeContext.tsx`
...
export function ThemeProvider({ children }: { children: ReactNode }) {
  const [currentTheme, setCurrentTheme] = useState<Theme>(() => {
    ...
    return allThemes.find(t => t.id === "classic-dark") || allThemes[0];
  });
...
## File 2: `client/src/data/themes.ts`
...
export const allThemes: Theme[] = [
  ...
  createTeamTheme("jets", "Winnipeg Jets", "nhl", "https://content.sportslogos.net/logos/1/3050/full/z9qyy9xqoxfjbpqnwrk8u4l95.png", "from-blue-900 via-slate-800 to-blue-900", "from-blue-600 to-white", "bg-blue-700"),
  createTeamTheme("coyotes", "Utah Hockey Club", "nhl", "https://content.sportslogos.net/logos/1/6756/full/utah_hockey_club_logo_primary_20247253.png", "from-blue-900 via-black to-blue-900", "from-blue-600 to-black", "bg-blue-700"),
...
## File 3: `client/src/components/ThemeSelector.tsx`
  ...
  return (
    <Dialog open={open} onOpenChange={setOpen}>
      <DialogTrigger asChild>
        <Button variant="outline" size="icon" data-testid="button-theme-selector">
          ...
          {Object.entries(themesByCategory).map(([category, themes]) => (
            <AccordionItem key={category} value={category}>
              <AccordionTrigger className="text-sm">
                {categoryLabels[category as ThemeCategory]} ({themes.length})
              </AccordionTrigger>
              <AccordionContent>
                <div className="grid grid-cols-2 gap-2">
                  {themes.map((theme) => (
                    <button
                      ...
                      className={`p-2 rounded-md text-left text-xs flex items-center gap-2 border ${
                    ...
                    >
                      <div
                        className={`w-6 h-6 rounded-full bg-gradient-to-br ${theme.colors.primary}`}
                      />
...
function MyComponent() {
  ...
  return (
    <div className={`${currentTheme.colors.background} ${currentTheme.colors.textPrimary}`}>
      <div className={`bg-gradient-to-r ${currentTheme.colors.primary} p-4`}>
...
```typescript
// For sports teams, use createTeamTheme helper:
createTeamTheme(
  "team-id",           // Unique ID
  "Team Name",         // Display name
  "nfl",               // Category
  "https://logo.png",  // Logo URL (watermark)
...
// For custom themes, use full object:
{
  id: "my-theme",
  name: "My Custom Theme",
  category: "nature",
  watermark: "https://image.jpg",  // Optional
  colors: {
    primary: "from-color-900 via-color-800 to-color-900",
    secondary: "from-color-600 to-color-600",
    accent: "bg-color-600",
    background: "bg-slate-950",
    cardBg: "bg-slate-900",
    textPrimary: "text-white",
...
This blueprint includes **150+ themes** covering:
- 3 Classic themes (Light, Dark, Pure Black)
- 32 NFL teams
- 30 MLB teams  
- 30 NBA teams
- 32 NHL teams
